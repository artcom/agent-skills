...
| [design-diff](/skills/design-diff/SKILL.md)                     | Verify a rendered screen against its Figma export with the `design-diff` CLI — a pass/fail gate plus a ranked diagnosis (font-metrics mismatch, uniform offset, DOM-attributed hotspots) instead of a bare pixel-diff percentage |
