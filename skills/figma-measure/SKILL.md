...
see the [`design-diff`](../design-diff/SKILL.md) skill for the full command/config reference.
`design-diff verify`, then `design-diff explain <scenario>` when that fails, reports a mean channel
delta, a best-fit pixel offset **per font size** (the gradient that identifies a variable-font
mismatch, see `figma-to-react` §4 Tokens), and the strongest differences attributed to DOM elements.
Without it, the three mechanisms above are the fallback — but diff programmatically either way,
never by inspection.
